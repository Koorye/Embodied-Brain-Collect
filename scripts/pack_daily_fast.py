...
def _probe_session_index(sd: Path, video_slots, full: bool) -> dict | None:
    ...
    # (位姿特征 = observation.<role|deviceN>_pose,角色名来自
    # role_serial_map;hand_pose 是 40 手部关节,不算位姿设备)
    n_dev = sum(1 for k in present
                if k.startswith("observation.") and k.endswith("_pose")
                and k != "observation.hand_pose")
    ...
    # 视频流发现(完整性检查在 QC,与 pack_daily 同口径)
    for slot, entries in video_slots.items():
        for npz_name, ts_key, mp4_name, suffix in entries:
            ...
            if not len(in_win):
                continue
            # 容器完整性与帧数覆盖只在 QC 检查一次(见 pack_daily),probe
            # 仅做发现:窗口内有时间戳的视频槽位进特征集合
...
def _build_specs(sessions: list[dict], video_slots, common: set[str]) -> dict:
    ...
    # 与 pack_daily.build_feature_specs 的音频段一致
    if MICROPHONE_KEY in common and MICROPHONE_KEY not in specs:
        for info in sessions:
            mic = load_microphone_index(info["dir"])
            if mic is None:
                continue
...
def main(argv: list[str] | None = None) -> int:
    ...
    with ProcessPoolExecutor(max_workers=args.workers) as pool:
        ...
        n_video_jobs = sum(len(e) for e in video_slots.values())
        with ProcessPoolExecutor(
            ...
            for i, info in enumerate(sessions):
                ...
                for slot, entries in video_slots.items():
                    for npz_name, ts_key, mp4_name, suffix in entries:
                        ...
                        vid_futs[(i, suffix)] = video_pool.submit(
            ...
            try:
                ...
                for i, info in enumerate(sessions):
                    ...
                    for entries in video_slots.values():
                        for _npz_name, _ts_key, _mp4_name, suffix in entries:
                            res = vid_futs[(i, suffix)].result()
