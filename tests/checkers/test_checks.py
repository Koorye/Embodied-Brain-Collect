...
def test_sanity_dup_level_is_configurable_per_modality(ctx):
    """EMG 的时间线应严格逐帧递增:重复即拟合被拒、退回到达时间戳,
    要按 ERROR 报(其他模态保持 WARN 默认)。"""
    c = ctx(series={"s": np.array([0.0, 1.0, 1.0, 2.0])})
    out = C.TimestampSanity(series="s", dup_level="ERROR").run(c)
    assert levels(out) == ["ERROR"]
    assert "重复" in messages(out)
...
# =============================================================================
# TimestampGap × marker window — head/tail coverage
# =============================================================================

WINDOW = {"t0": 10.0, "t1": 20.0}


def test_gap_edge_silent_when_stream_covers_window(ctx):
    """流贴着窗口边缘有样本(窗口外还有数据):无 finding,余量≈0。"""
    t = np.r_[np.arange(9.0, 10.0, 0.01), np.arange(10.0, 21.0, 0.01)]
    out = C.TimestampGap(series="s").run(ctx(series={"s": t}, window=WINDOW))
    assert out.findings == []
    assert out.stats["s"]["head_margin_s"] == pytest.approx(0.0, abs=0.02)
    assert out.stats["s"]["tail_margin_s"] == pytest.approx(0.0, abs=0.02)


def test_gap_flags_late_start_and_early_stop_as_error(ctx):
    """开录晚 1.5s、提前 2.0s 收尾:一条 ERROR 覆盖两端。"""
    t = np.concatenate([np.arange(11.5, 15.0, 0.01),
                        np.arange(18.0, 15.05, -0.01)[::-1]])
    out = C.TimestampGap(series="s").run(ctx(series={"s": t}, window=WINDOW))
    assert levels(out) == ["ERROR"]          # 0.05s 内部缺口低于阈值,不报
    assert "首端缺数据 1.50s" in messages(out)
    assert "尾端缺数据 2.00s" in messages(out)


def test_gap_edge_uses_same_threshold_as_internal(ctx):
    """首尾与内部缺口同阈值(min_s=0.1):0.05s 的边缘相位不算缺。"""
    t = np.arange(10.05, 19.95, 0.01)        # 首尾各差 0.05s,正常采样相位
    out = C.TimestampGap(series="s").run(ctx(series={"s": t}, window=WINDOW))
    assert out.findings == []


def test_gap_edge_requires_window(ctx):
    """无 marker 窗口(整段录制)时没有边缘可言,行为不变。"""
    t = np.arange(0.0, 5.0, 0.01)
    out = C.TimestampGap(series="s").run(ctx(series={"s": t}, window=None))
    assert out.findings == []
    assert "head_margin_s" not in out.stats["s"]


def test_tracking_gap_inherits_edge_check(ctx):
    """TrackingGap(按有效样本)同样吃到首尾检查:tracker 从头就丢跟 → ERROR。"""
    from embodied_brain_collect.checkers.position import TrackingGap
    t = np.arange(12.0, 20.0, 0.001)         # 首端缺 2s
    valid = np.ones((len(t), 3), bool)       # 3 台设备全程有效
    c = ctx(series={"combined": t}, window=WINDOW,
            arrays={"positions_m": np.random.default_rng(0).normal(
                0, 0.1, (len(t), 3, 3)), "valid": valid})
    out = TrackingGap().run(c)
    # 逐设备各一条,每条都带首端缺口
    assert set(levels(out)) == {"ERROR"} and len(out.findings) == 3
    assert all("首端缺数据 2.00s" in f.message for f in out.findings)
