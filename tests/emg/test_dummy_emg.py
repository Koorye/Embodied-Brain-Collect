...
class TestDummyEmg(BaseTest):
    ...
    def _update(self, rec, elapsed):
        ...
        if emg:
            n = len(emg)
            emg_ts = [i / max(n - 1, 1) * elapsed for i in range(n)] if n > 1 else [0.0]
            sl = self._rolling(emg_ts, elapsed, window=5.0)
        ...
        if gyro:
            n = len(gyro)
            gyro_ts = [i / max(n - 1, 1) * elapsed for i in range(n)] if n > 1 else [0.0]
            sl = self._rolling(gyro_ts, elapsed, window=5.0)
        ...
        if accel:
            n = len(accel)
            accel_ts = [i / max(n - 1, 1) * elapsed for i in range(n)] if n > 1 else [0.0]
            sl = self._rolling(accel_ts, elapsed, window=5.0)
