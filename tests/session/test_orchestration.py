...
from embodied_brain_collect.session.troubleshooting import (
    format_failure_help, self_check_flow, slot_category)
...
# =============================================================================
# run_base.ask_next —— 防误触的严格输入
# =============================================================================

def test_ask_next_rejects_empty_and_wrong_letters(monkeypatch):
    from embodied_brain_collect.session.run_base import ask_next

    answers = iter(["", "x", "keep", "n"])
    monkeypatch.setattr(builtins, "input", lambda *_: next(answers))
    assert ask_next() == "next"

    answers = iter(["r"])
    monkeypatch.setattr(builtins, "input", lambda *_: next(answers))
    assert ask_next() == "rerun"

    answers = iter(["Q"])
    monkeypatch.setattr(builtins, "input", lambda *_: next(answers))
    assert ask_next() == "quit"   # 大小写不敏感
...
def test_collect_layers_empty_nqc_and_clean(tmp_path):
    from embodied_brain_collect.session.run_base import collect_runs
    ...
    s = collect_runs([tmp_path / "01-empty", tmp_path / "02-eye-bad",
...
def test_print_summary_renders_marks_and_streams(tmp_path, capsys):
    from embodied_brain_collect.session.run_base import collect_runs, print_summary
    ...
    s = collect_runs([tmp_path / "01-empty", tmp_path / "02-clean"],
                     {"01-empty": "rerun", "02-clean": "kept"})
    print_summary(s, title="测试")
...
# =============================================================================
# run_qc —— QC 成功后必须走到 html 开关(回归:曾引用未定义的 checker_cfg)
# =============================================================================

def test_run_qc_completes_and_writes_reports(tmp_path):
    """QC 一成功就 NameError 的回归:checker_cfg 曾只被内联进 qc_session,
    下面的 html 开关又引用它 —— 每次成功的 QC 都会崩,qp.html 永远出不来。"""
    from embodied_brain_collect.session.launcher import run_qc

    rc = run_qc(tmp_path)          # 空目录 = 空报告,恰好走到 html 开关
    assert rc == 0                 # NameError 会让本行直接抛异常
    assert (tmp_path / "qc_report.json").is_file()
    # qc.html 不强求:空报告时 build_page 可能失败,由 run_qc 内部 try 兜住


def test_ask_next_fail_quit(monkeypatch):
    """n/r/f/q 中的 f = 失败退出(自 prod 合并)。"""
    from embodied_brain_collect.session.run_base import ask_next

    monkeypatch.setattr(builtins, "input", lambda *_: "f")
    assert ask_next() == "fail_quit"
