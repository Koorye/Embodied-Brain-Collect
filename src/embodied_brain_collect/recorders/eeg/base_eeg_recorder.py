...
# 成对斜率是 O(n²)。BrainCo 包时钟一场可达 1 万+点,纯 Python 双循环会
# 卡几十秒,launcher join(15s) 会把进程杀掉,eeg.npz 就此丢了。
# 先均匀抽点估斜率,再在全量上做内点/polyfit。
MAX_PAIRWISE_POINTS = 400
...
def _fit_eeg_to_pc(eeg_t_s: np.ndarray, marker_t_pc: np.ndarray) -> dict:
    ...
    xs, ys = xr, yr
    if xs.size > MAX_PAIRWISE_POINTS:
        idx = np.unique(np.round(
            np.linspace(0, xs.size - 1, MAX_PAIRWISE_POINTS)).astype(np.int64))
        xs, ys = xs[idx], ys[idx]
    # dx[i,j] = xs[j]-xs[i],与原先 i<j 的 (xr[j]-xr[i]) 一致
    dx = xs[None, :] - xs[:, None]
    dy = ys[None, :] - ys[:, None]
    ...
    span = dxi >= 0.5
    s = dyi[span] / dxi[span]
    s = s[(s >= SLOPE_COARSE[0]) & (s <= SLOPE_COARSE[1])]
    if s.size == 0:
        return _fail("no plausible slope between any two events")

    a0 = float(np.median(s))
...
class BaseEegRecorder(BaseRecorder):
    ...
    def _setup(self) -> None:
        """开录时清掉 open 期间攒下的数据 —— Blackrock 的 SDK 回调从
        connect 起就在产数,独立 run() 场景下这些属于录制前数据。launcher
        两段式路径上,commit 会经基类 _discard_preface 再清一次并重置锚点。"""
        super()._setup()
        self._blocks.clear()
        self._total_samples = 0
