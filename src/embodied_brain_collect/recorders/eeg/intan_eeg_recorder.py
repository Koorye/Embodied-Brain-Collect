...
class IntanEegRecorder(BaseEegRecorder):
    ...
    def _open(self) -> bool:
        try:
            ...
            self._log(f"[eeg:intan] open failed — {self._open_error}")
            self._close_sockets()
            return False

    def _connect_rhx(self) -> None:
        """命令口握手 + 通道使能 + 波形流;任何失败都以异常抛出。"""
        cfg = self.config
        try:
        ...
        try:
            ...
            raise ConnectionError(f"读不到采样率({resp!r})")
        if self._sample_rate <= 0:
            raise ConnectionError(f"采样率异常: {self._sample_rate!r}")
        ...
        if not ch_names:
            raise ConnectionError(
                "通道列表为空 — 检查 ports/channels_per_port/channels 配置")
    ...
    def _data_connect(self) -> None:
        """连数据口;start_data_server 时先确保服务器在 connect 状态。

        失败以 ConnectionError 抛出,由 _open 统一收口。
        """
        ...
        try:
            ...
            raise ConnectionError(
                f"连不上 RHX 波形数据口 {self.config.host}:"
