...
class BlackrockEegRecorder(BaseEegRecorder):
    ...
    def _open(self) -> bool:
        try:
            ...
            self._log(f"[eeg:blackrock] open failed — {self._open_error}")
            self._teardown_session()
            return False

    def _connect_sdk(self) -> None:
        """建立 SDK 会话;任何失败都以异常抛出,由 _open 统一收口。"""
        try:
            from pycbsdk import ChannelType, DeviceType, SampleRate, Session
        except Exception as exc:  # ImportError or a broken cffi/dll load
            raise ConnectionError(
                f"pycbsdk 不可用 — pip install pycbsdk "
                f"({type(exc).__name__}: {exc})")
        try:
            device_type = DeviceType[self.config.device_type.strip().upper()]
        except KeyError:
            raise ConnectionError(
                f"未知 device_type: {self.config.device_type!r} "
        ...
        if not session.running:
            raise ConnectionError(
                "设备未发现/未连接 — 检查 NSP 网络(默认 192.168.137.x)"
                "或本机 Central")
        ...
        if auto:
            ...
            if group is None:
                raise ConnectionError(
                    f"auto_enable_group={auto} 不是合法速率"
            ...
            try:
                ...
                raise ConnectionError(
                    f"auto_enable_group 配置失败 — "
                    f"{type(exc).__name__}: {exc}")

        group, chids = self._pick_group(session)
        if not chids:
            raise ConnectionError(f"采样组 {group} 的通道列表为空")
