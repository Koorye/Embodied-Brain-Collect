...
class NeonEyeAsyncRecorder(BaseEyeRecorder):
    ...
    def __init__(self, config: EyeRecorderConfig):
        ...
        self._scene_writer_task: asyncio.Task | None = None
    ...
    def _reset_stream_state(self) -> None:
        # commit 清 buffer 时,采集队列里还压着预热期投喂的样本,一并丢弃,
        # 否则下一个 _drain 会把 pre-commit 数据写进刚清空的缓冲
        self._gaze_q.clear()
        self._imu_q.clear()
        ...
        try:
            ...
            async with Device(address=device_info.addresses[0], port=device_info.port) as dev:
                ...
                while True:
                    ...
                    if w is not None and w.done() and not w.cancelled() \
                            and w.exception() is not None:
                        self._log(f"[eye:neon] scene writer died — "
                                  f"{type(w.exception()).__name__}: "
            ...
            if self._start_evt.is_set():
                await self._finish_scene_video()    # 先排空尾帧,再关 mp4
                self._teardown()    # 正式开录过:关流 + 保存
            else:
                self._close()       # 中止路径:不写空数据文件
            self._record_rc = rc

    async def _finish_scene_video(self) -> None:
        """停表后、``_teardown`` 关 ffmpeg 前,等写盘任务把队列里的尾帧写完。

        ``writer.close()`` 是阻塞调用,会连事件循环一起堵住;若不等写盘
        任务排空就直接 close,在途的那一帧会被 close 掉——视频恰好比
        时间戳少 1 帧。先 await 排空(该帧已写完并记账),close 与在途
        write 的竞态即消失。
        """
        task = self._scene_writer_task
        self._scene_writer_task = None
        if task is None:
            return
        try:
            await asyncio.wait_for(asyncio.shield(task), timeout=10.0)
        except asyncio.TimeoutError:
            self._log("[eye:neon] scene writer 排空超时 — eye.mp4 可能缺尾帧",
                      level="WARNING")
            task.cancel()
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            self._log(f"[eye:neon] scene writer failed: "
    ...
    async def _scene_task(self, url, stop):
        ...
        try:
            ...
            # 不在这里 raise 写盘任务的异常:它的死活由主循环监督,收尾
            # 排空由 _finish_scene_video 负责;这里 raise 只会把取消路径搅乱
            await asyncio.gather(writer_task, return_exceptions=True)

    async def _scene_writer(self, queue, stop):
        """Drain the decoded-frame queue into eye.mp4 at its own pace.

        Timestamps are recorded HERE, AFTER the frame write has completed,
        so eye.mp4 stays 1:1 with scene_timestamps: a frame that failed to
        write (or was lost to close-racing an in-flight write) is never
        counted.  记账在写完成后,时间戳数永远等于已写入帧数。
        """
        loop = asyncio.get_running_loop()
