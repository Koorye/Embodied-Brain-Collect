...
class DummyPositionRecorder(BasePositionRecorder):
    ...
    def _close(self) -> None:
        self._log("[position:dummy] stopped")

    def _poll(self, ts):
        time.sleep(0.016)
        r = 0.5
        # Mirror the real recorder's schema (timestamps + validity + device
        # identity), otherwise anything consuming a dummy session sees a
        # stream with no timeline and cannot align it to the others.
        ...
        self._acc_arr("valid", np.array([True]))
