...
``{key}_timestamps.txt`` / the timestamps NPZ stay 1:1 with the container's
frame indices — when the writer falls behind, the oldest queued frames are
dropped instead of letting the recording drift behind real time.
"""
...
class BaseCameraRecorder(BaseRecorder):
    ...
    def __init__(self, config):
        ...
        self._ts_files: dict[str, object] = {}    # open append handles
    ...
    def _acc_ts(self, cam: str, ts: float) -> None:
        super()._acc_ts(cam, ts)
        if not self.config.session_dir:
            return
        try:
            f = self._ts_files.get(cam)
            if f is None:
                out_dir = Path(self.config.session_dir) / self.output_dir
                out_dir.mkdir(parents=True, exist_ok=True)
                f = (out_dir / f"{cam}_timestamps.txt").open("a", encoding="utf-8")
                self._ts_files[cam] = f
            f.write(f"{ts}\n")
            f.flush()
        except OSError as exc:
            self._warn_write(cam, exc)
    ...
    def _save(self) -> None:
        """Stop the write worker (flush tail, finalize mp4s), close
        timestamp streams, write the timestamps NPZ and the final report."""
        if not self.config.session_dir:
            return

        # The writer threads own the mp4s and the timestamp streams — stop
        # them BEFORE closing anything they write to.
        self._stop_write_worker()

        for cam, f in self._ts_files.items():
            try:
                f.close()
            except OSError:
                pass
        self._ts_files.clear()
        ...
        if ts_payload:
            out_dir = Path(self.config.session_dir) / self.output_dir
            out_dir.mkdir(parents=True, exist_ok=True)
            ts_path = out_dir / f"{self.output_dir}_timestamps.npz"
