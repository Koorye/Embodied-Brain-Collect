...
- `audio_enabled: true`：请求并保存头环麦克风；设 false 保持原六路行为。启用后若服务端未提供音频，open 失败。
...
- `microphone_timestamp_basis=alsa_capture_pointer_first_sample_estimate` 时，逐包另存 `microphone_stamp_ns`（本包首采样的 CLOCK_REALTIME 纳秒时刻，由服务端 ALSA 采样指针回推并经写入端逐包校验）、`microphone_has_capture_timestamp`，以及 `microphone_capture_monotonic_ns` / `microphone_alsa_pointer_monotonic_ns` / `microphone_alsa_available_frames` / `microphone_realtime_minus_monotonic_ns` / `microphone_clock_mapping_uncertainty_ns` 等校验用时间字段；basis 为 legacy（`read_complete_not_hardware_capture`）时 `microphone_stamp_ns` 恒 0、`microphone_has_capture_timestamp` 恒 False。

每包 320 点/20 ms/640 字节。握手后等待录制期间的音频与图像一样被丢弃；因此第一包设备样本位置可能不为零，而 WAV 偏移从零开始。不要把两者混用。

音频时间保存服务端 `read_complete_ns`，不使用 `header.stamp_ns=0` 伪造采集时间，也不退化为 Windows 接收时间。它是服务端读完一包的时间，不是硬件采样时间；不能据此声称相机和音频硬件级同步。capture basis 下另有按 ALSA 采样位置校验过的首采样时刻 `microphone_stamp_ns`，这才是硬件采集时刻的估计。打包端（`pack_daily.load_microphone_index`）只认 `microphone_stamp_ns`，不做 read_complete 回推估计；没有逐块有效采集时间戳的会话（旧数据）打包时跳过音频。
