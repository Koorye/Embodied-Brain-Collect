...
class NetEgoHeadbandRecorder(BaseEgoHeadbandRecorder):
    ...
    def __init__(self, config: EgoHeadbandRecorderConfig):
        ...
        # recorder into its own process, and the socket is created in _open()
        # there, never in the parent.
    ...
    def _on_camera(self, i: int, payload: bytes, t: float) -> None:
        if not payload:
            ...
            return
        # JPEG passthrough: hand the compressed frame straight to the async
        # writer, which lets ffmpeg decode (mjpeg_cuvid / GPU) + re-encode
        # (hevc_nvenc) it — NO cv2.imdecode on the CPU.  Four 1920x1088@30
        # fisheye streams otherwise saturate the CPU (libx265 alone runs ~17
        # fps per stream).  arr_jpeg stamps the stream only for frames the
        # writer actually commits, so timestamps stay 1:1 with the mp4.
        self.arr_jpeg(self._cam_stem(i), t, payload)
