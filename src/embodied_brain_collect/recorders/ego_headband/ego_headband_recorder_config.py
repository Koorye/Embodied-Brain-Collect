...
HEVC pipeline); ``encoder="gpu"`` (the default) does that decode+encode on the
GPU (``mjpeg_cuvid`` + ``hevc_nvenc``), falling back to CPU only if unusable.
...
class EgoHeadbandRecorderConfig(BaseRecorderConfig):
    ...
    crf: int = 23
    preset: str = "medium"
    encoder: str = "gpu"         # "gpu"(默认)=mjpeg_cuvid+hevc_nvenc,探测不可用则告警回退CPU;
                                 # 可选 "auto"(静默回退)/"cpu"(强制libx265)
