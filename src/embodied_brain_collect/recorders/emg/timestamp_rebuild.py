...
_MIN_ANCHORS = 5            # fewer than this and the slope is mostly noise
_OUTLIER_SIGMA = 3.0
_OUTLIER_ROUNDS = 3         # 迭代剔离群的最大轮数(单轮的 σ 会被离群撑大)
_THEILSEN_CAP = 400         # Theil–Sen 配对抽样的锚点上限(限制 O(n²))
...
def _robust_fit(k: np.ndarray, t_rel: np.ndarray) -> tuple[float, float]:
    """Theil–Sen 初估:斜率 = 两两配对斜率的中位数,截距 = 残差中位数。

    普通最小二乘会被单个调度卡顿的锚点倾斜,而随后 3σ 剔离群的阈值又
    恰恰被这些离群撑大 —— 先用中位数拿到一条不被离群拖动的线。锚点多时
    等距抽样到 ≤400 个,限制 O(n²) 配对数。
    """
    n = k.size
    if n > _THEILSEN_CAP:
        idx = np.unique(np.linspace(0, n - 1, _THEILSEN_CAP).astype(np.int64))
        k, t_rel = k[idx], t_rel[idx]
        n = k.size
    ...
    for i in range(n - 1):
        ...
        keep = dx > 0
        if keep.any():
            slopes.append((t_rel[i + 1:] - t_rel[i])[keep] / dx[keep])
    a = float(np.median(np.concatenate(slopes))) if slopes else 0.0
    b = float(np.median(t_rel - a * k))
    return a, b
...
def rebuild(emg_ts: np.ndarray, emg_sn: np.ndarray,
    ...
    try:
        ...
        # The span follows both streams' extents, not the EMG anchors alone:
        # the first transmitted frame can be an IMU frame (k < 0 — the open
        # read happens to start with IMU), which otherwise reads as a
        # one-frame deficit and fails a perfectly clean recording.
        lo = min(0, int(k_imu[0])) if k_imu.size else 0
        hi = (max(int(k_emg[-1]), int(k_imu[-1]))
              if k_imu.size else int(k_emg[-1]))
        sent = hi - lo + 1
        ...
        # Theil–Sen 初估 —— 单个调度卡顿的锚点拖不动中位数斜率;再迭代 σ
        # 剔离群、最小二乘精修。单轮 3σ 的 σ 会被离群本身撑大,多离群时
        # 容易剔不干净而拒拟合(短会话尤甚)。
        t_ref = float(a_ts[0])
        k_fit, t_fit = a_k, a_ts - t_ref
        a, b = _robust_fit(k_fit, t_fit)
        res = t_fit - (a * k_fit + b)
        n_out = 0
        for _ in range(_OUTLIER_ROUNDS):
            ...
            if not removed or int(keep.sum()) < _MIN_ANCHORS:
                break
            # res 与 k_fit/t_fit 始终一一对应:剔离群是在当前保留集上收缩,
            # 掩码只能作用于保留集 —— 索引回全量锚点会长度错位。
            k_fit, t_fit = k_fit[keep], t_fit[keep]
            a, b = _fit(k_fit, t_fit)
            res = t_fit - (a * k_fit + b)
            n_out += removed
