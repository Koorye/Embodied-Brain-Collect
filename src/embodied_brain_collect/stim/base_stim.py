...
# 全屏黑底、红点注视、浅灰正文、深灰提示、红色"请继续" —— 共用同一套视觉语言
BG = (0, 0, 0)
FIX_COLOR = (220, 0, 0)
TEXT_COLOR = (230, 230, 230)
HINT_COLOR = (120, 120, 120)
CONTINUE_COLOR = (220, 0, 0)  # 「请按回车键继续」等被试操作提示
...
class BaseStim:
    ...
    def add_common_args(ap: argparse.ArgumentParser, over: dict) -> None:
        ...
        ap.add_argument("--display", type=int,
                        ...
                        help="启动屏幕索引(0=主屏;双屏时刺激屏通常为 1)。"
                             "全屏/窗口化与分辨率按 configs/displays.yaml "
                             "里该屏的设置")
        ...
        ap.add_argument("--sync-udp-port", type=int,
                        default=int(over.get("sync_udp_port", 0) or 0),
                        help="把同一份 marker 再抄到此 UDP 端口(BrainCo 软件"
                             "同步,与 recorders.yaml 的 eeg.sync_udp_port "
                             "一致;0=关闭)")
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        if display >= n_displays:
            print(f"[stim] 配置的屏幕 {display} 不存在(共 {n_displays} 块)"
                  "— 用主屏")
            display = 0
        from ..session.config import display_settings
        ...
        self.screen = pygame.display.set_mode(size, flags, display=display)
        # 实际画布尺寸回填进 args —— 范式里按 args.width/height 居中排版
        # (sync_test 等),全屏时= 该屏分辨率,窗口化时= 配置的窗口尺寸
        args.width, args.height = self.screen.get_size()
        ...
        self.font_banner = pygame.font.Font(
            font_path, max(64, int(args.font_size * 1.35)))
        ...
        self.marker = MarkerSender(
            ...
            sync_udp_port=int(getattr(args, "sync_udp_port", 0) or 0),
        )
        ...
        self.continue_color = CONTINUE_COLOR
        self.aborted = False
    ...
    def _wait_keys(self, keys, draw_fn=None) -> bool:
        """等待 ``keys`` 中任一键;期间可用 draw_fn 重绘。False = 已中止。"""
        pg = self.pygame
        want = tuple(keys)
    ...
    def _wait_space(self, draw_fn=None) -> bool:
        ...
        return self._wait_keys((self.pygame.K_SPACE,), draw_fn)

    def _wait_enter(self, draw_fn=None) -> bool:
        """等待回车(主键盘或小键盘)。"""
        pg = self.pygame
        return self._wait_keys((pg.K_RETURN, pg.K_KP_ENTER), draw_fn)
