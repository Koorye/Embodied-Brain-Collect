...
Replaces E-Prime's InLine marker code in pure Python.  Sends the same byte to:
  ...
  - optionally a second UDP copy to the EEG recorder's ``sync_udp_port``
    (BrainCo 软件同步:帽子没有硬件 TTL 事件流,recorder 在线按最近一包
    插值出事件供心跳显示;落盘仍以离线包时钟拟合为准)
...
class MarkerSender:
    ...
    def __init__(
        ...
        self.sync_udp_host = sync_udp_host
        self.sync_udp_port = int(sync_udp_port or 0)
    ...
    def mark(self, code: int, tag: str, *, ttl: bool = True) -> float:
        """Emit one marker on all paths, return the wall-clock emit time.

        ``ttl=False`` 只走 UDP(重播中途按键打标用):避免 20ms 串口脉冲
        卡住视频播放线程,UDP tag 仍完整记录该次按键。

        ParallelBox 时序:线路平时保持在 0 —— 打开时置 0(构造)、每个码
        发完置 0(mark 尾部)、结束时置 0(close),因此每次写码都是
        0→code 的上升沿,放大器才能锁存(现场实测缺 0 沿码会收不到)。
        """
        if not 0 <= code <= 255:
        ...
        if self._sock is not None:
            raw = (f"EVT|trial={self._trial}|tag={tag}|code={code}"
                   f"|t_eprime_ms={t_eprime_ms}|t_sent_pc={t_wall!r}"
                   ).encode("utf-8")
            dests = [(self.udp_host, self.udp_port)]
            if self.sync_udp_port > 0:
                dests.append((self.sync_udp_host, self.sync_udp_port))
            for dest in dests:
                try:
                    self._sock.sendto(raw, dest)
                except Exception as exc:  # noqa: BLE001
                    if self.verbose:
                        print(f"[marker] UDP send to {dest} failed: {exc}")

        if ttl and self._sers:
