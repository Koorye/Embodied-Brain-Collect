...
class SyncTestStim(BaseStim):
    ...
    def _draw_read(self, text: str) -> None:
        ...
        screen = self.screen
        self._clear()
        head_text = (f"{self.scene_task} — 请记住想象序列"
                     if self.scene_task else "请记住想象序列")
        head = self.font_small.render(head_text, True, self.text_color)
