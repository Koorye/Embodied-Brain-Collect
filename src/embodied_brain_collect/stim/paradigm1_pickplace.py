...
class TrialSpec:
    task_id: int | None
    task_name: str
    scene_title: str = ""    # 图纸模式:场景名+图纸编号,替代"任务 #N"标题
...
class Paradigm1Stim(BaseStim):
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        if args.environment:
            self.trial = TrialSpec(task_id=None,
                                   task_name=self.scene_task,
                                   scene_title=self.scene_title)
            return
        if args.task_id is None:
            raise SystemExit("[stim] 需要 --task-id 或 --environment 之一")
    ...
    def _draw_instruction(self) -> None:
        ...
        header_text = (self.trial.scene_title
                       if self.trial.scene_title
                       else f"任务 #{self.trial.task_id}")
        header = self.font_title.render(header_text,
    ...
    def _draw_status(self, trial_n: int, hint: str) -> None:
        ident = (self.trial.scene_title
                 if self.trial.scene_title
                 else f"task {self.trial.task_id}")
        header = f"{ident}  run {self.run_no}  #{trial_n}"
    ...
    def run_flow(self) -> None:
        ...
        while not self.aborted:
            ...
            print(f"[trial #{trial_n}] scene='{trial.scene_title}' "
...
def main(argv: list[str] | None = None) -> int:
    ...
    ident = stim.trial.scene_title or f"task_id={stim.trial.task_id}"
    print(f"[stim] {ident} task_name={stim.trial.task_name!r} "
          f"(Esc 结束，空格重复本任务)")
    return stim.run()
