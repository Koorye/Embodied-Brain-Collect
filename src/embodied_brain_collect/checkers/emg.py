...
class EmgChecker(BaseChecker):
    ...
    ``emg_timestamps``/``imu_timestamps`` 应为收尾时从序列号拟合出的逐帧
    时间(严格递增,见 ``timestamp_rebuild``)。出现重复即说明拟合被拒、
    文件里仍是读取到达时间(~99% 重复,无法与其他流逐帧对齐)—— 因此
    两个系列的重复时间戳都按 ERROR 报,而不是其他模态的 WARN;拟合成功
    时严格递增,不会产生任何提示。
    """
    ...
    checks = [
        ts_checks("emg", dup_level="ERROR"),
        ts_checks("imu", dup_level="ERROR"),
        SnGap(),
        MadOutlier("emg_data"),
