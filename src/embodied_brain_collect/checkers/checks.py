...
class TimestampSanity(BaseCheck):
    ...
    ``dup_level`` sets the severity of duplicate timestamps: WARN where
    duplicates are a known artifact of some recorders (position), ERROR for
    streams whose timeline is supposed to be strictly per-frame (EMG —
    duplicates there mean the sequence-number fit was refused and the
    read-arrival stamps shipped instead, ~99% of the series identical).
    """
    ...
    def run(self, ctx: CheckContext) -> CheckOutput:
        ...
        if n_dup:
            out.findings.append(self.finding(
                self.dup_level,
                f"{n_dup} 个重复时间戳({n_dup / s.dt.size:.1%})",
...
class TimestampGap(BaseCheck):
    ...
    窗口模式下首尾同样在检查范围内:间隔检测只能看到两个样本**之间**的洞,
    第一条样本之前、最后一条之后没有 diff 可看 —— 开录晚于 RUN_START、
    提前于 RUN_END 收尾原本是结构性盲区。这里拿窗口裁剪后的首/末样本对
    marker 窗口边缘做对照,超出同一阈值(> ``min_s``)按同口径报 ERROR。
    """
    ...
    def run(self, ctx: CheckContext) -> CheckOutput:
        ...
        for subject, t, med in self.scan_targets(ctx):
            ...
            # 首/尾对照窗口边缘(窗口裁剪保证 margin >= 0);无窗口时没有
            # 边缘可言,既不检查也不出数
            head = float(t[0] - win["t0"]) if win is not None else None
            tail = float(win["t1"] - t[-1]) if win is not None else None
            edges = ([(side, m) for side, m in
                      (("首端", head), ("尾端", tail)) if m > thr]
                     if win is not None else [])
            out.stats[subject] = {"n_gaps": len(segs), "total_gap_s": total}
            if win is not None:
                out.stats[subject]["head_margin_s"] = head
                out.stats[subject]["tail_margin_s"] = tail
            if edges:
                out.stats[subject].update({f"{s}_gap_s": m for s, m in edges})
                out.findings.append(self.finding(
                    "ERROR",
                    "窗口" + "、".join(
                        f"{side}缺数据 {m:.2f}s(> 阈值 {thr:.3f}s)"
                        for side, m in edges)
                    + f" — 开录晚于 RUN_START / 早于 RUN_END 结束{self.note}",
                    subject=subject, threshold=thr,
                    observed=max(m for _, m in edges),
                    detail={side: m for side, m in edges},
                    spans=[Span(float(win["t0"] if side == "首端" else t[-1]),
                                m, f"窗口{side}缺 {m:.2f}s")
                           for side, m in edges]))
            if not segs:
                continue
...
@dataclass(frozen=True)
class ContainerIntegrity(_VideoCheck):
    """ffprobe 能否解封装该容器 —— 报错即容器损坏(moov 缺失/未正常收尾)。

    cv2 的 decode 对半截文件可能"打开成功但 0 帧"甚至静默给出部分帧,
    对"坏容器"不够敏锐;ffprobe 按包计数,容器索引残缺会直接报错并给出
    原因(moov atom not found 等)。这类文件必然进不了打包(pack 侧的
    完整性门用同一 ffprobe 口径),在 check 阶段就标 ERROR,让 pack 的
    "QC 无 ERROR"门槛把它挡在数据集之外。
    """
    ...
    def run(self, ctx: CheckContext) -> CheckOutput:
        ...
        try:
            nb = ffprobe_count(p)
        except Exception as exc:  # noqa: BLE001 — 任何探测失败都是坏容器
            out.findings.append(self.finding(
                "ERROR",
                f"视频容器无法读取(ffprobe): {p.name} — {exc}"
                "(文件损坏或录制未正常收尾,不会进入打包)",
                field=p.name))
            return out
        out.stats.update({"file": p.name, "n_packets": int(nb)})
        return out
...
def ts_checks(series: str | None = None, *,
              ...
              dup_level: str = "WARN") -> list[BaseCheck]:
    ...
    ``dup_level`` is forwarded to :class:`TimestampSanity` — EMG raises it to
    ERROR because a duplicate there means the arrival-stamp fallback shipped.
    """
    out: list[BaseCheck] = [
        TimestampSanity(series=series, dup_level=dup_level),
