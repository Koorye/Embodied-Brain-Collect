...
@dataclass(frozen=True)
class DeviceCount(BaseCheck):
    """Tracker count against the rig's expected number.

    A tracker that is off — or simply not yet seen by SteamVR — at recorder
    open time is absent from the device axis entirely: every per-device
    check then runs on a healthy-looking stream, and only the count itself
    reveals the loss.  Too many devices is the same class of fault in
    mirror image: the dev0/dev1/… column order shifts between sessions.
    """
    ...
    def run(self, ctx: CheckContext) -> CheckOutput:
        pos = np.asarray(ctx.arr("positions_m"), dtype=np.float64)
        n = int(pos.shape[1]) if pos.ndim == 3 else 1
        out = CheckOutput()
        out.stats["n_devices"] = n
        if n == self.expected:
            return out
        if n < self.expected:
            msg = (f"仅 {n}/{self.expected} 台 tracker — 开录前有设备未连接,"
                   "缺失的设备轴整段无数据")
        else:
            msg = (f"{n} 台 tracker,应 {self.expected} 台 — 多出的设备会让"
                   "device 序号跨会话错位")
        out.findings.append(self.finding(
            "ERROR", msg, field="positions_m",
            threshold=float(self.expected), observed=float(n),
            detail={"n_devices": n, "expected": self.expected}))
        return out
