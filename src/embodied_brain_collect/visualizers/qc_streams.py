...
def rows_wristband(z, d: Path, t0: float, opt) -> tuple[list, list]:
    """BLE 生理手环:压力/PPG/IMU/血氧/温度 + 1 Hz 状态帧.

    与 checker 同源的四条设备时钟序列;压力、PPG 逐通道细行(通道间量纲
    差异大,叠在一张图上读不出信息),IMU 走 xyz_rows,血氧/温度/电量画
    慢变量。
    """
    rows: list = []
    t_frame = _rel(z, "frame_timestamps", t0)
    t_p = _rel(z, "pressure_timestamps", t0)
    t_ppg = _rel(z, "ppg_timestamps", t0)
    t_imu = _rel(z, "imu_timestamps", t0)

    if t_frame is not None:
        rows += timing_rows(t_frame, "frame")
    if t_p is not None:
        rows += timing_rows(t_p, "pressure")
    if t_ppg is not None:
        rows += timing_rows(t_ppg, "ppg")
    if t_imu is not None:
        rows += timing_rows(t_imu, "imu")

    if t_p is not None:
        p = z.get("pressure_raw")
        if p is not None and len(p) == t_p.size:
            r = row("压力 (raw)", [series(t_p, p, label="pressure", slot=1)],
                    h=48, src="pressure")
            if r:
                rows.append(r)
    if t_ppg is not None:
        ppg = z.get("ppg_raw")
        names = ([str(x) for x in z.get("ppg_channel_names")]
                 if z.get("ppg_channel_names") is not None else None)
        if ppg is not None and ppg.ndim == 2 and len(ppg) == t_ppg.size:
            for c in range(ppg.shape[1]):
                label = names[c] if names and c < len(names) else f"ch{c}"
                r = row(f"PPG {label}",
                        [series(t_ppg, ppg[:, c], label=label, slot=1)],
                        h=40, src="ppg")
                if r:
                    rows.append(r)
    if t_imu is not None:
        for key, label, unit in (("gyro_rad_s", "陀螺", "rad/s"),
                                 ("accel_m_s2", "加速度", "m/s²")):
            a = z.get(key)
            if a is not None and len(a) == t_imu.size:
                rows += xyz_rows(label, t_imu, a, unit=unit, src="imu")
        for key, label, unit in (("spo2_percent", "血氧", "%"),
                                 ("temperature_c", "温度", "°C")):
            a = z.get(key)
            if a is not None and len(a) == t_imu.size:
                r = row(label, [series(t_imu, a, label=label, slot=3,
                                       unit=unit)], h=40, src="wristband")
                if r:
                    rows.append(r)
    t_status = _rel(z, "status_host_timestamps", t0)
    if t_status is not None:
        b = z.get("battery_percent")
        if b is not None and len(b) == t_status.size:
            r = row("电量", [series(t_status, b, label="battery", slot=3,
                                    unit="%")], h=32, src="wristband")
            if r:
                rows.append(r)
    return rows, []
...
def rows_ego_headband(z, d: Path, t0: float, opt) -> tuple[list, list]:
    ...
    it draws like the standalone camera stream: its frame clock always, plus
    luminance / frame-diff traces and thumbnails decoded from the mp4 when the
    caller asked for frames.  The IMUs draw gyro/accel exactly like the eye
    and EMG streams do.  Camera and IMU counts are read from the field names,
        ...
        if opt.frames and mp4.is_file():
            ...
            if v.t_samp is not None and v.t_samp.size > 2:
                ...
                if r:
                    rows.append(r)
                r = row(f"{cam} 帧间差异",
                        [series(v.t_samp, v.diffs, label="帧差", slot=2)],
                        h=48, src=cam)
                if r:
                    rows.append(r)
    ...
    for j in imus:
        ...
        if t is None:
            continue
        rows += timing_rows(t, f"imu{j}")
        for key, label, unit in ((f"imu{j}_gyro", f"IMU{j} 陀螺", "rad/s"),
                                 (f"imu{j}_accel", f"IMU{j} 加速度", "m/s²")):
            a = z.get(key)
...
EXTRACTORS: tuple[tuple[str, object], ...] = (
    ...
    ("wristband", rows_wristband),
    ("ego_headband", rows_ego_headband),
    ("marker", rows_marker),
    ("cam", rows_camera),
)
