...
def apply_filter(data, fs: float, preset: FilterPreset) -> np.ndarray:
    ...
    through unfiltered, so the page always has something drawable and a
    QC run can never die on one odd channel.
    """
    data = np.asarray(data)
    out = data.astype(np.float64, copy=True)
    if (_sp_signal is None or data.ndim != 2 or data.shape[0] < 91
            or fs <= 2 * preset.hi):
        return out
    try:
        ...
        return out
    # 0.5 Hz 高通的稳定时间是秒级:sosfiltfilt 默认 padlen 只有 ~45 样本,
    # 滤波器在头尾远未进入稳态,带直流偏置/慢漂移的信号会在头尾产生巨大
    # 瞬态(表现为"滤波放大了头尾数据")。两步处理:
    #   1. 先减每通道均值 —— 带通本来就去直流,减掉后延拓从零电平开始,
    #      头尾的摆动幅度只取决于漂移,不再取决于偏置本身;
    #   2. odd 延拓加长到最低频 3 个周期,滤波器进入真实数据前就已稳定。
    bad = ~np.isfinite(data).all(axis=0)
    if bad.all():
        return out
    good = ~bad
    out[:, good] -= np.nanmean(out[:, good], axis=0)
    settle = int(round(3.0 * fs / preset.lo))
    padlen = min(max(45, settle), (data.shape[0] - 1) // 2)
    try:
        out[:, good] = _sp_signal.sosfiltfilt(sos, out[:, good], axis=0,
                                              padlen=padlen)
