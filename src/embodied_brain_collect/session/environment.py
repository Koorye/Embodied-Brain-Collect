...
``config.yaml``(task/scene 定义)和一批图纸 png(文件名按 ``_`` 分割
第 3 段为 num,如 ``餐桌_做三明治_0001_r1.png``;旧命名
``NNNN_comboNNN_rN.png`` 兼容)。
每次采集随机抽一张**还没用过**的图纸全屏显示,采集员照着图纸摆放实物,
按 s + Enter 关闭图纸后才正式开录;采集成功的图纸由调用方记入**该场景目录
...
class Environment:
    ...
    def __init__(self, root: Path | str | None = None):
        ...
        # 进程内按场景目录缓存 config.yaml / used.yaml 的解析结果 ——
        # 队列打印会对每张图纸调 scene_task,几百张图纸时不缓存要
        # 逐张读盘(实测 ~46ms/张)。文件在本进程内只经 mark_used 变更
        # (写后同步缓存),跨进程不存在陈旧问题。
        self._config_cache: dict[Path, dict] = {}
        self._used_cache: dict[Path, dict] = {}
    ...
    def load_used(self, rel: str) -> dict[str, dict]:
        ...
        if directory in self._used_cache:
            return self._used_cache[directory]
        try:
            data = yaml.safe_load(
        ...
        self._used_cache[directory] = used
        return used
    ...
    def mark_used(self, rel: str, session: str = "") -> None:
        ...
        (path.parent / self.ledger_name).write_text(
            ...
            encoding="utf-8")
        self._used_cache[path.parent] = used
    ...
    def scene_config(self, rel: str) -> dict:
        ...
        try:
            ...
            cfg = {}
        self._config_cache[directory] = cfg
        return cfg

    @staticmethod
    def drawing_info(rel: str) -> dict:
        """文件名 → {num, combo, rep};不匹配则 {}。

        新命名:带 overview 的总览图(placements_overview 等)优先排除;
        其余按 ``_`` 分割取前 3 段,第 3 段全数字即 num
        (``餐桌_做三明治_0001_r1`` → num=1),combo/rep 新命名不含,
        记 None —— placements.csv 按 num 匹配行,combo 以 CSV 列为准。
        旧命名 ``[前缀_]NNNN_comboNNN_rN`` 优先按正则解析,num/combo/rep
        齐全(``0001_combo006_r1``、``餐桌_保鲜盒_0001_combo015_r2``)。
        """
        stem = Path(rel).stem
        if "overview" in stem:
            return {}
        m = re.match(r"(?:.+?_)?(\d+)_combo(\d+)_r(\d+)$", stem)
        if m:
            return {"num": int(m[1]), "combo": int(m[2]), "rep": int(m[3])}
        parts = stem.split("_")
        if len(parts) >= 3 and parts[2].isdigit():
            return {"num": int(parts[2]), "combo": None, "rep": None}
        return {}
    ...
    def scene_layout(self, rel: str) -> dict:
        ...
        列名前缀为物体名)。返回 {task_name, scene, notes, objects} ——
        与开录时固化进 session meta.yaml 的顶层字段一一对应(num/combo/rep
        由图纸文件名自带,不再重复存);notes = config.yaml ``scene.notes``
        (新版权面备注,旧 config 无此键为 None);
    ...
    def show(self, rel: str, *, display: int | None = None) -> str:
        """展示一张图纸,按 ``s + Enter`` 确认后关闭(pygame 实现)。

        只有先按 ``s`` 再按 ``Enter`` 才算确认(返回 "done");其他按键
        一律无效(提示后原地等待),Esc/关窗未经确认视为取消(返回
        "abort")。图纸显示在哪块屏由 stim.yaml 的 ``drawing_display``
        (缺省回退 ``display``)决定;全屏/窗口化与分辨率跟随
        displays.yaml 里该屏的设置。
        """
        import pygame
        from ..stim.base_stim import _FONT_CANDIDATES, _find_font

        from .config import display_settings, load_stim
        stim_cfg: dict = load_stim() or {}
        if display is None:
        ...
        font = pygame.font.Font(
            ...
            max(20, sh // 50))
        caption = f"环境图纸 {rel} — 照图摆放,按 s + Enter 开始采集(Esc 取消)"
        ...
        try:
                ...
                for ev in pygame.event.get():
                    if ev.type == pygame.QUIT:
                        ...
                        if ev.key == pygame.K_ESCAPE:
                            ...
                            pressed_s = False
                            hint = "输入无效 — 请按 s + Enter 开始采集"

                screen.fill((24, 24, 24))
                screen.blit(img, img_rect)
                screen.blit(font.render(caption, True, (255, 255, 255)),
                            (24, 12))
                if hint:
            ...
            pygame.display.quit()      # 销毁全屏窗口
            pygame.display.init()      # 但模块保持可用 —— 同进程下一张图纸还要再 set_mode
        return verdict
...
def show(rel: str, *, display: int | None = None) -> str:
    return _environment.show(rel, display=display)
