...
# _buf keys streamed line-by-line to text files (crash-safe timestamps).
_TXT_STREAM_KEYS = {"gaze_timestamps", "imu_timestamps", "scene_timestamps"}
...
class BaseEyeRecorder(BaseRecorder):
    ...
    def __init__(self, config):
        ...
        self._txt_files: dict[str, object] = {}   # open append handles
    ...
    def _acc(self, key: str, value: Any) -> None:
        super()._acc(key, value)
        if key not in _TXT_STREAM_KEYS or not self.config.session_dir:
            return
        try:
            f = self._txt_files.get(key)
            if f is None:
                out_dir = Path(self.config.session_dir) / self.output_dir
                out_dir.mkdir(parents=True, exist_ok=True)
                f = (out_dir / f"{key}.txt").open("a", encoding="utf-8")
                self._txt_files[key] = f
            f.write(f"{value}\n")
            f.flush()
        except OSError as exc:
            self._warn_write(key, exc)
    ...
    def _save(self) -> None:
        ...
        for f in self._txt_files.values():
            try:
                f.close()
            except OSError:
                pass
