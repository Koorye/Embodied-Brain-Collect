...
| `scripts/run_session_video.py` | **主控·video_rate（RLHF 视频打分）**：选子集（`configs/videos/<任务>/`，`config.yaml` 的 `task` = 想象内容）→ 按台账抽取配平组合 → **逐条录制**（每 trial 一次独立会话，评分 JSON 与视频副本都写进录制目录）→ 成功记子集台账 |
...
1. **图纸模式**：`configs/environments` 图纸池 + 全屏摆放确认（s+Enter）+
...
6. **`session/environment.py` 的 `Environment` 类**：图纸池/台账/全屏显示
   （matplotlib 中文字体自动选择、s+Enter 确认状态机）。
